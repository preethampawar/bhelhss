...
<div class="wrapper row1">
	<div id="header" class="clear">
		<div>
			<div class="fl_left">
				<?php echo $this->Html->link($this->Html->image('bhel_hss_logo.jpg', array('height'=>'120', 'border'=>'0')), '/admin/categories/', array('title'=>'BHEL Higher Secondary School Alumni', 'escape'=>false));?>
			</div>
			<div class='fl_left' style="margin: 40px 10px 0 10px;">
				<h1><?php echo $this->Html->link('BHEL HSS Alumni', '/admin/categories/', array('title'=>'BHEL Higher Secondary School Alumni'));?></h1>
			...
			<div class="fl_right" style="margin:35px 10px 0 0; text-align:right;">
				...
				?>
					<ul>
						<li><?php echo $this->Html->link('Register', '/users/register', array('style'=>'border-bottom:1px dotted #ffffff;'));?></li>
				...
				?>
					<ul>
						<?php
						if($this->Session->read('Auth.User.has_completed_profile')) {
						?>
				...
				<?php
				$showSearch=true;
				if($this->Session->check('User') and !$this->Session->read('Auth.User.has_completed_profile')) {
					$showSearch = false;
				}
				if($showSearch) {
				?>
			...
			<?php
			if($this->Session->check('Auth.User') and $this->Session->read('Auth.User.has_completed_profile')) {
			?>
			<div style="margin:20px 10px 0 10px; color:#eee;">
				<div class="floatLeft">
					<p> Welcome, <?php echo $this->Session->read('Auth.User.name');?>
					&nbsp;&nbsp;|&nbsp;&nbsp;
					Batch: <?php echo $this->Session->read('Auth.User.batch');?> (10<span style="font-style:italic; font-size:80%;">th</span> class)</p>
				</div>
			...
			<?php
			if($this->Session->check('User') and ($this->Session->read('Auth.User.admin'))) {
			?>
			<div style="margin:20px 10px 0 10px; color:#eee;">
		...
		<div>
			...
			<div class="fl_right" style="margin:35px 10px 0 0; text-align:right;">
				...
				if($this->Session->check('Auth.User')) {
					...
					if($this->Session->check('Auth.User')) {
						?>
						<h2>Welcome, <?php echo $this->Session->read('Auth.User.name');?></h2>
						<?php
					}
					?>
					<ul>
