...
<div id="editCategoryForm">
	<h1 class="floatLeft">Edit Category - <?php echo $categoryInfo['Category']['name'];?></h1>
	...
	<?php 
		...
		if(isset($this->data['Category']['parent_id'])) {
			$default = $this->data['Category']['parent_id'];
		}
		...
		<div>
			<div class="floatLeft" style="width:250px; float:left; margin-right:20px;text-align:center;">
				<div class="corner categorySelectionDiv" style="height:320px;">
					<?php 
					$parentCategory = (isset($categories[$categoryInfo['Category']['parent_id']])) ? $categories[$categoryInfo['Category']['parent_id']] : '';
					?>
					<div style="margin:0px; padding:0px; text-align:left;">
					Parent Category: <?php echo $parentCategory;?>
					</div>
					<?php
					echo $this->Form->input('Category.parent_id', array('label'=>false, 'options'=>$categories, 'escape'=>false, 'empty'=>'-- None --', 'default'=>$default, 'size'=>'17', 'style'=>'border:0px; padding:0px; background:transparent;'));?>
				</div>
			</div>
			<div class="floatLeft" style="width:500px; float:left; margin-right:20px;">
				<div class="corner contentDiv" style="height:320px;">		
					<?php	
					echo $this->Form->input('Category.name', array('label'=>'Category Name', 'required'=>true, 'value'=>html_entity_decode($this->data['Category']['name'], ENT_QUOTES)));
					?>
					<div>
						...
						<br/>
						<?php	echo $this->Form->submit('Save Changes &nbsp;&raquo;', array('escape'=>false)); ?>
					</div>
				</div>				
			</div>
			<div class="clear"></div>
